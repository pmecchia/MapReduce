...
def hashing(input_file,ip_list):
    hash_dic={}
    file=open(input_file,'r',encoding='utf-8').readlines()
    for line in file: # reading each line
        encode_string=line.encode('utf-8')
        hash_code = str(int.from_bytes(hashlib.sha256(encode_string).digest()[:8], 'little'))
...
if __name__ == '__main__':
    ...
    if int(sys.argv[1])==0:
        ...
        file_words=open(input_file,'r',encoding='utf-8').read().split()
    ...
    ################################################################################
        ...
        reduce_file.close()
        """
        with open("/tmp/pmecchia-20/reduces/"+hostname+".txt","a") as f:
            print(reduces_dic, file=f)